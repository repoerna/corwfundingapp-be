...
func (h *userHandler) RegisterUser(c *gin.Context) {
	...
	if err != nil {
		response := helper.APIResponse("Register account failed", http.StatusInternalServerError, "error", err.Error())
...
func (h *userHandler) Login(c *gin.Context) {
	...
	if err != nil {
		response := helper.APIResponse("Login failed", http.StatusInternalServerError, "error", err.Error())
		c.JSON(http.StatusInternalServerError, response)
		return
	}
...
func (h *userHandler) CheckEmailAvailability(c *gin.Context) {
	...
	if err != nil {
		...
		response := helper.APIResponse("Email check failed", http.StatusUnprocessableEntity, "error", errorMessages)
		c.JSON(http.StatusUnprocessableEntity, response)
		return
	}
	...
	if err != nil {
		errorMessages := gin.H{"error": "Server error"}
		response := helper.APIResponse("Email check failed", http.StatusUnprocessableEntity, "error", errorMessages)
		c.JSON(http.StatusUnprocessableEntity, response)
		return
	}
